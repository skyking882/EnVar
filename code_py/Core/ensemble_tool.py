...
def ensemble_generator(Nen, u_e, Y=1000, variance=0.005, rng=None):
    ...
    # --- FIX: Handle Low-Dimensional Systems (Nen > N_dofs) ---
    if Nen > N_dofs:
        # If requested ensemble size > system rank, we cannot extract Nen orthogonal modes.
        # Instead, we directly sample Nen members from the generated pool P_hat.
        # The scaling factor (1/sqrt(Y)) is maintained for consistency with the SVD path.
        P = P_hat[:, :Nen] * (1.0 / math.sqrt(Y))
    else:
        # Original SVD Projection Logic (for Nen <= N_dofs)
